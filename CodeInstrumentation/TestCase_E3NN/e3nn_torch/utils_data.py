...
def get_node_attr(atomic_numbers, n):
    ...
    z += temp * n
    return torch.from_numpy(np.array(z, dtype = np.float32))
...
def build_data(id, structure, qpts, band_structure, r_max, descriptor='mass'):
    ...
    data = Data(id = id,
                ...
                qpts = torch.tensor(qpts, dtype = torch.float32),
...
def create_virtual_nodes_vvn(structure0, vnelem, edge_src0, edge_dst0, edge_shift0):
    ...
    numb = len(positions0)
    lattice = torch.from_numpy(structure.cell.array.copy()).unsqueeze(0).to(default_dtype)
...
def build_data_vvn(id, structure, qpts, gphonon, r_max, vnelem='Fe', descriptor='mass'):
    ...
    positions = torch.from_numpy(structure.get_positions().copy()).to(default_dtype)
    numb = len(positions)
    lattice = torch.from_numpy(structure.cell.array.copy()).unsqueeze(0).to(default_dtype)
    ...
    node_deg = get_node_deg(edge_dst, len(x))
    y = torch.from_numpy(gphonon/1000).unsqueeze(0).to(default_dtype)
    data = Data(id = id,
                pos = positions,
                lattice = lattice,
                symbol = symbols,
                ...
                edge_vec = torch.tensor(edge_vec, dtype=default_dtype),
                edge_len = torch.tensor(edge_len, dtype=default_dtype),
                qpts = torch.from_numpy(qpts).unsqueeze(0).to(default_dtype),
                gphonon = torch.from_numpy(gphonon).unsqueeze(0).to(default_dtype),
                r_max = r_max,
                # ucs = None,
                numb = numb)
